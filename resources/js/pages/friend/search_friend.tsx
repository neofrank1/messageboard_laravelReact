...
import { searchUserTypes } from "@/types/friendRequestTypes";
import { useState } from "react";
import { UserRoundPlus } from 'lucide-react';

export default function SearchFriendPage({ users, addedFriendIds = [] }: { users: searchUserTypes[], addedFriendIds?: number[] }) {
                                            ...
                                            <div className="grid grid-cols-2">
                                                ...
                                                <div className="flex justify-end items-center gap-4">
                                                    <button 
                                                        ...
                                                        disabled={addedFriends.has(request.id)}
                                                    >
                                                        <UserRoundPlus />
                                                        {addedFriends.has(request.id) ? 'Friend Added' : 'Add Friend'}
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
